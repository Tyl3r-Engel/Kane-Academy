...
const { mentorProfile } = require('./controllers/mentorProfiles.js')
...
async function fakeMentorProfile() {
  ...
  for (var i = 1; i < 11; i++) {
    ...
    mentorProfile(mentor_id, about, (err, result) => {
      if (err) {
...
async function fakeSkills() {
  ...
  for (var i = 0; i < 30; i++) {
    ...
    skills(name, category, description, (err, result) => {
      if (err) {
...
async function fakeReviews() {
  ...
  for (var i = 1; i < 11; i++) {
    ...
    let time = new Date();
    reviews(mentor_id, learner_id, skill_id, rating, body, time, (err, result) => {
      if (err) {
